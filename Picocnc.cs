...
{
    /// <summary>
    /// Returns true if a PicoGK viewer is active. In headless mode
    /// (e.g., web backend), Library.oViewer() returns null and all
    /// viewer operations must be skipped.
    /// </summary>
    public static bool bHasViewer()
    {
        try { return Library.oViewer() != null; }
        catch { return false; }
    }
    ...
    {
        Log("PicoCNC -- CNC Machine Generator");
        ...
        // In headless mode Library.oViewer() returns null -- guard all calls
        if (bHasViewer())
        {
        ...
        Log("PicoCNC construction complete.");
        ...
        Log("All exports complete. PicoCNC finished.");
    }
}
